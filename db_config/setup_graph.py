...
graph_name = "grafo_academico"
if db.has_graph(graph_name):
    graph = db.graph(graph_name)
else:
    graph = db.create_graph(graph_name)

edge_name = "requisitos"
if not graph.has_edge_definition(edge_name):
    graph.create_edge_definition(
        edge_collection=edge_name,
        from_vertex_collections=["cursos"],
        to_vertex_collections=["cursos"],
    )
    print(f"Aresta '{edge_name}' criada para pré-requisitos entre cursos.")

print("Grafo de pré-requisitos configurado.")
